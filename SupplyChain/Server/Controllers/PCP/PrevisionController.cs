...
namespace SupplyChain.Server.Controllers
{
    ...
    public class PrevisionController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<Producto>>> BuscarPorCG_PROD(string CG_PROD)
        {
            ...
            {
                lDesProd = await _context.Productos.Where(p => p.CG_PROD == CG_PROD).ToListAsync();
            }
            if (lDesProd == null)
            {
                return NotFound();
        ...
        public async Task<ActionResult<List<Producto>>> BuscarPorDES_PROD(string DES_PROD)
        {
            ...
            if (_context.Productos.Any())
            {
                lDesProd = await _context.Productos.Where(p => p.DES_PROD == DES_PROD).ToListAsync();
            }
            if (lDesProd == null)
            {
                return NotFound();
        ...
        public async Task<ActionResult<List<Producto>>> BuscarProductoPrevision(string CG_PROD, string DES_PROD, int Busqueda)
        {
            ...
            if (DES_PROD == "Vacio")
            {
                if (_context.Productos.Any())
                {
                    lContiene = await _context.Productos.Where(p => p.CG_PROD.Contains(CG_PROD)).Take(Busqueda).ToListAsync();
                }
                if (lContiene == null)
                {
                    return NotFound();
            ...
            else if (CG_PROD == "Vacio")
            {
                if (_context.Productos.Any())
                {
                    lContiene = await _context.Productos.Where(p => p.DES_PROD.Contains(DES_PROD)).Take(Busqueda).ToListAsync();
                }
                if (lContiene == null)
                {
                    return NotFound();
            ...
            else if (CG_PROD != "Vacio" && DES_PROD != "Vacio")
            {
                if (_context.Productos.Any())
                {
                    lContiene = await _context.Productos.Where(p => p.CG_PROD.Contains(CG_PROD) && p.DES_PROD.Contains(DES_PROD)).Take(Busqueda).ToListAsync();
                }
                if (lContiene == null)
                {
                    return NotFound();
